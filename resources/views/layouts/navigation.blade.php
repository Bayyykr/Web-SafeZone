                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center gap-2 px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            @if ($authUser?->profile_photo)
                                <img class="h-8 w-8 rounded-full object-cover" src="{{ asset('storage/' . $authUser->profile_photo) }}" alt="Foto profil {{ $authUser->name }}">
                            @else
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="flex items-center gap-3">
                    @if ($authUser?->profile_photo)
                        <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/' . $authUser->profile_photo) }}" alt="Foto profil {{ $authUser->name }}">
                    @else
                    ...
                    @endif
                    <div>
                        <div class="font-medium text-base text-gray-800">{{ $authUser?->name ?? 'User' }}</div>
